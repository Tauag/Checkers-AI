...
public class MLThirtyFiveRepBoard implements CheckersGameState{
		private String _playerTurn;
		private String _winner;
		private ThirtyFiveRepCheckerPiece[] _set;
		
		public MLThirtyFiveRepBoard(){
			_playerTurn = "Black";
			_set = new ThirtyFiveRepCheckerPiece[35];
			initBoard();
		}
		
		public MLThirtyFiveRepBoard(String turn, ThirtyFiveRepCheckerPiece[] set){
			_playerTurn = turn;
			_winner = null;
			_set = set;
		}
		...
		public CheckersGameState result(Move x){
			...
			return new MLThirtyFiveRepBoard(next, newset);
		}
...
//			System.out.println(heuristicScore);
	...
	public int analyzeML(String player, MLControllerConstants playercontrols, MLWeightConstants playerweights) {
		...
		if(playercontrols._BACK)
			heuristicScore += (backheuristic(player) * playerweights._BACK);
		if(playercontrols._CENT)
			heuristicScore += (centheuristic(player) * playerweights._CENT);
		if(playercontrols._CNTR)
			heuristicScore += (cntrheuristic(player) * playerweights._CNTR);
		if(playercontrols._KCENT)
			heuristicScore += (kcentheuristic(player) * playerweights._KCENT);
		if(playercontrols._MOB)
			heuristicScore += (mobheuristic(player) * playerweights._MOB);
		if(playercontrols._POLE)
