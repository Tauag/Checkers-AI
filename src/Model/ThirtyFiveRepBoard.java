...
public class ThirtyFiveRepBoard implements CheckersGameState {
	...
	public List<Move> actions() {
		...
		for (int i = 0; i < 35; i++) {
			if (_set[i] != null && _set[i].getColor().equals(_playerTurn) && isValid(i)) {
				for (Move mv : hasJumpMoves(i, _set[i], _set[i].isKing(), i)) { // for
																				// all
																				// possible
																				// jump
																				// moves
																				...
																				// each
																				// piece
					temp = fillJumpMove(i, _set[i], _set[i].isKing(), mv, i); // temp
																				// is
																				// a
																				// list
																				// of
	 ...
	 */
	public List<Move> hasJumpMoves(int location, ThirtyFiveRepCheckerPiece piece, boolean kinged,
			int originalLocation) {
		List<Move> possibleMoves = new LinkedList<Move>();
		if (piece.getColor().equals("Black")) {
			// The logic works like this:
			// First check if the current piece's location and its destination
			// location is valid
			// Second check that the space it is jumping over is filled and it
			// is a piece of the opposing team
			// Finally check that its destination is empty and therefore allows
			// a jump to occur OR if the destination is the same and the jump is
			// cyclical
			if (isValid(location + 8) && isValid(location + 4) && _set[location + 4] != null
					&& _set[location + 4].getColor().equals("White"))
				if (_set[location + 8] == null || ((location + 8) == originalLocation))
					possibleMoves.add(new Move(location, (location + 8), (location + 4)));
			if (isValid(location + 10) && isValid(location + 5) && _set[location + 5] != null
					&& _set[location + 5].getColor().equals("White"))
		...
		if (piece.getColor().equals("White")) {
			if (isValid(location - 10) && isValid(location - 5) && _set[location - 5] != null
					&& _set[location - 5].getColor().equals("Black"))
				if (_set[location - 10] == null || ((location - 10) == originalLocation))
					possibleMoves.add(new Move(location, (location - 10), (location - 5)));
			if (isValid(location - 8) && isValid(location - 4) && _set[location - 4] != null
					&& _set[location - 4].getColor().equals("Black"))
				if (_set[location - 8] == null || ((location - 8) == originalLocation))
					possibleMoves.add(new Move(location, (location - 8), (location - 4)));
			if (kinged) {
				if (isValid(location + 8) && isValid(location + 4) && _set[location + 4] != null
	 ...
	 */
	public List<Move> fillJumpMove(int location, ThirtyFiveRepCheckerPiece target, boolean kinged, Move move,
			int originalLocation) {
		...
		if (target.getColor().equals("Black")) {
			...
			for (Move mv : hasJumpMoves(move.getnewcoordinate(), target, isKing, originalLocation))
				if (!killedalready(mv, move)) {
					...
					retlist.addAll(fillJumpMove(nextLocation, target, isKing,
							new Move(move.old_coordinate, mv.getnewcoordinate(), new LinkedList<Integer>(move._kills)),
							originalLocation));
				}
		} else {
	...
	/*
	 * Positions less than 0, greater than 34 are off limits Positions 8, 17 and
	 * 26 are not valid
	 */
	...
	/*
	 * Basically its hasJumpMoves but returns immediately when a jump is
	 * available Marks a piece as "Active" if it has jumps
	 */
	public boolean isActive(int location, ThirtyFiveRepCheckerPiece piece) {
		if (piece.getColor().equals("Black")) {
			if (isValid(location + 8) && isValid(location + 4) && _set[location + 4] != null
					&& _set[location + 8] == null)
				if (_set[location + 4].getColor().equals("White"))
					return true;
			if (isValid(location + 10) && isValid(location + 5) && _set[location + 5] != null
					&& _set[location + 10] == null)
	...
	/*
	 * Basically hasSingleMoves but returns true on first possible move, false
	 * otherwise
	 */
	...
	/*
	 * (non-Javadoc) Aggragates the total heuristic score Takes the weights from
	 * the constants file
	 */
	public int analyze(String player) {
		...
		if (ControllerConstants._RELATIVEKINGS)
			heuristicScore += (relativekingsheuristic(player) * WeightConstants._RELATIVEKINGS);
		if (ControllerConstants._AGGRO)
			heuristicScore += (aggroheuristic(player) * WeightConstants._AGGRO);

		return heuristicScore;
	}

	/*
	 * The parameter is credited with 1 for each passive man in the 5th and 6th
	 * rows (in the direction of the player) and debited 1 for each passive man
	 * in the 3rd and 4th rows
	 */
	...
	/*
	 * -1 if there are no kings, positions 6 and 28 are not filled with active
	 * pieces and neither of them are filled with passive pieces
	 */
	...
	/*
	 * The parameter is credited with 1 if there are no active kings on the
	 * board and if the two bridge squares (0 and 2, or 32 and 34) in the back
	 * row are occupied by passive pieces.
	 */
	...
	/*
	 * The parameter is credited with 1 for each of the following squares: 10,
	 * 11, 14, 15, 19, 20, 23 and 24 which is occupied by a passive man.
	 */
	...
	/*
	 * The parameter is credited with 1 for each of the following squares: 10,
	 * 11, 14, 15, 19, 20, 23 and 24 that is either currently occupied by an
	 * active piece
	 */
	...
	/*
	 * The parameter is credited with 1 for each of the following squares: 10,
	 * 11, 14, 15, 19, 20, 23 and 24 which is occupied by a passive king
	 */
	...
	/*
	 * The parameter is credited with 1 for each square to which the active side
	 * could move one or more pieces in the normal fashion, disregarding the
	 * fact that jump moves may or may not be available
	 */
	...
	/*
	 * The parameter is credited with 1 for each passive man that is completely
	...
	/*
	 * The parameter is credited with one for each piece it has more than the
	 * opponent. It is debited for each piece the opponent has more than the
	 * player.
	 */
	 ...
	 */
	public int relativekingsheuristic(String player) {
	...
	/*
	 * This gives a point or every piece of the player's in a spot adjacent to a
	 * spot adjacent to an opponent's piece.
	 */
	public int aggroheuristic(String player) {
		int total = 0;
